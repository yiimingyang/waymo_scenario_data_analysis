...
def calculate_surrounding_vehicle_with_lane_change_gap(scenario):
    for agent in scenario.agents_list:
        ...
        while state_index < state_number:
            ...
            # stage2: find the preceeding and following vehicles on the same lane on each timestep or
            # find the preceding and following vehciles on the lane before thge current lane or the lane 
            # behind the current lane
            
            map_matching_lane_id_list = []
            map_matching_lane_id_list.append(map_matching_lane_id)
            
            # find the exist lane of current lane, the exist lane must be a straight lane
            front_straight_lane_id = None         
            if len(map_matching_lane.exit_lanes_list) != 0:
                for next_lane_id in map_matching_lane.exit_lanes_list:
                    next_lane = scenario.center_lane_dict[next_lane_id]
                    if next_lane.is_straight or next_lane.is_straight_turn:
                        front_straight_lane_id = next_lane_id
                        if front_straight_lane_id != None and front_straight_lane_id != "None" and front_straight_lane_id != "":
                            map_matching_lane_id_list.append(front_straight_lane_id)
                        
            # find the entry lane of current lane, the entry lane must be a straight lane
            back_straight_lane_id = None
            if len(map_matching_lane.entry_lines_list) != 0:
                for before_lane_id in map_matching_lane.entry_lines_list:
                    before_lane = scenario.center_lane_dict[before_lane_id]
                    if before_lane.is_straight or before_lane.is_straight_turn:
                        back_straight_lane_id = before_lane_id                        
                        if back_straight_lane_id != None and back_straight_lane_id != "None" and back_straight_lane_id != "":
                            map_matching_lane_id_list.append(back_straight_lane_id) 
            ...
            for surrounding_vehicle in scenario.agents_list:  
                ...
                if len(surrounding_vehicle_map_matching_lane_id_list) != 0:
                    ...
                    if surrounding_vehicle_map_matching_lane_id in map_matching_lane_id_list:                        
